...
from ped_agent.vision.inference import (
    TrackAssignment,
    assemble_pixel_tracks,
    build_bytetrack_config,
)
from ped_agent.vision.model_registry import ModelManifestRegistry, ModelWeightsMismatchError
...
def test_model_registry_resolves_relative_weights_and_verifies_hash(tmp_path: Path) -> None:
    ...
    assert registry.manifest_sha256("mixed-flow-v1") == hashlib.sha256(
        loaded.model_dump_json(exclude={"weights_path"}).encode()
    ).hexdigest()

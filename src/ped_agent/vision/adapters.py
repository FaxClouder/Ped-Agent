...
from ped_agent.vision.contracts import KeypointObservation, ModelManifest, SemanticClass
from ped_agent.vision.runner import DecodedDetection, TrackedDetection, VideoFrame
    ...
    def __init__(self, video_path: Path, *, cv2_module: Any | None = None):
        if cv2_module is None:
            try:
                ...
                raise RuntimeError(
                    "Install ped-agent-core[vision] to read video frames"
                ) from exc
        ...
        from ped_agent.vision.contracts import VideoMetadata
...
class UltralyticsDetector:
    ...
    def detect(self, frame: Any) -> tuple[DecodedDetection, ...]:
        use_half = (
            self.device != "cpu" and self.manifest.inference.cuda_precision == "fp16"
        )
        results = self.model(
            frame,
            ...
            [
                [
                    *detection.bbox_xyxy,
                    ...
                    0
                    if ModelManifest.tracking_group(semantic_class) == "pedestrian"
                    else 1,
                ]
                for detection, semantic_class in zip(
                    detections, semantic_classes, strict=True
                )
            ],
            dtype=float,
        )

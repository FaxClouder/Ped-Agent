...
from ped_agent.vision.contracts import CalibrationMode, CalibrationReport, FrozenModel
...
def calibrate_charuco_images(
    ...
    *,
    ...
    if len(image_paths) < board_spec.minimum_views:
        raise ValueError(
            f"ChArUco calibration requires at least {board_spec.minimum_views} images"
        )
...
def solve_full_camera_from_ground_points(
    *,
    ...
    digest = sha256(
        matrix.tobytes() + rotation.tobytes() + translation.tobytes()
    ).hexdigest()[:16]
    report = CalibrationReport(

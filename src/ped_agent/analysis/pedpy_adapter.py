...
from ped_agent.vision.contracts import ProcessedWorldTrackSet, SceneProfile
...
def try_compute_pedpy_spatial(
    ...
    max_bin = max(
        int((item["frame"] / frame_rate) // profile.time_window_seconds)
        for item in rows
    )
    ...
    speed_data["x_index"] = (speed_data["x"] / profile.spatial_grid_metres).map(
        math.floor
    )
    speed_data["y_index"] = (speed_data["y"] / profile.spatial_grid_metres).map(
        math.floor
    )
    vector_field = tuple(
        VectorFieldCell(
...
def _try_voronoi(
    ...
    grouped = {
        int(time_bin): group
        for time_bin, group in individual.groupby("time_bin")
    }
    return tuple(
        VoronoiDensityPoint(
            time_bin=time_bin,
            ...
            mean_local_density_per_m2=(
                float(grouped[time_bin]["density"].mean())
                if time_bin in grouped
                else 0.0
            ),
            max_local_density_per_m2=(
                float(grouped[time_bin]["density"].max())
                if time_bin in grouped
                else 0.0
            ),
        )

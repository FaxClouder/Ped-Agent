"""Gold retrieval metrics, acceptance gates, and configuration activation."""
...
class EvaluationAcceptanceConfig(BaseModel):
    ...
    minimum_ndcg_at_k: float = Field(default=0.0, ge=0, le=1)
    minimum_locator_hit_rate: float = Field(ge=0, le=1)
    maximum_non_official_leakage: float = Field(ge=0, le=1)
    maximum_recall_regression: float = Field(default=0.0, ge=0, le=1)
    maximum_mrr_regression: float = Field(default=0.0, ge=0, le=1)
    maximum_ndcg_regression: float = Field(default=0.02, ge=0, le=1)
...
async def evaluate_retriever(
    ...
    *,
    ...
    for question in questions:
        result = await retriever.retrieve(question.query, limit=k)
        rankings[question.question_id] = [
            (str(item.resource_id), str(item.locator)) for item in result.items
        ]
    return evaluate_rankings(questions, rankings, k=k)
...
def audit_evaluation(
    ...
    *,
    ...
    if report.ndcg_at_k < config.minimum_ndcg_at_k:
        errors.append(f"nDCG@{config.k} is below {config.minimum_ndcg_at_k}")
    if report.locator_hit_rate < config.minimum_locator_hit_rate:
        errors.append(f"locator hit rate is below {config.minimum_locator_hit_rate}")
    if non_official_leakage > config.maximum_non_official_leakage:
        errors.append(f"non-official leakage exceeds {config.maximum_non_official_leakage}")
...
def compare_with_baseline(
    ...
    if baseline is None:
        return EvaluationComparisonReport(candidate=candidate, errors=())
    errors: list[str] = []
    if baseline.recall_at_k - candidate.recall_at_k > config.maximum_recall_regression:
        errors.append("candidate Recall@K regresses beyond the configured tolerance")
    if baseline.mrr - candidate.mrr > config.maximum_mrr_regression:
        errors.append("candidate MRR regresses beyond the configured tolerance")
    if baseline.ndcg_at_k - candidate.ndcg_at_k > config.maximum_ndcg_regression:
        errors.append("candidate nDCG@K regresses beyond the configured tolerance")
...
def publish_retrieval_config(
    ...
    *,
    ...
    acceptance: EvaluationAcceptanceReport,
    comparison: EvaluationComparisonReport,
) -> bool:
    activate = acceptance.is_compliant and comparison.is_compliant
    catalog.register_retrieval_config(
        config_id,
        payload,
        {

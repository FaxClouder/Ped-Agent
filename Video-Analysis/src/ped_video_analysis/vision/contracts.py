...
class DetectorManifest(FrozenModel):
    ...
    def require_business_classes_and_contact_points(self) -> DetectorManifest:
        ...
        required_names = {name for names in self.contact_keypoints.values() for name in names}
...
class TrackerManifest(FrozenModel):
    tracker_id: str = Field(pattern=r"^[a-z0-9][a-z0-9._-]+$")
    name: str = Field(min_length=1)
    version: str = Field(min_length=1)
    backend: Literal["bytetrack"]
    settings: ByteTrackDefaults = Field(default_factory=ByteTrackDefaults)


class ModelManifest(DetectorManifest):
    """Resolved runtime manifest combining one detector with one tracker."""

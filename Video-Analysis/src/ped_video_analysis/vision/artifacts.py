        ...
        *,
        ...
        tracks = tuple(
            PixelTrack(
                ...
                observations=tuple(sorted(grouped[track_id], key=lambda item: item.frame_index)),
            )
        ...
        for row in rows:
            ...
            grouped.setdefault(track_id, []).append(
                WorldTrackObservation(
                    frame_index=int(row["frame_index"]),
                    timestamp=float(row["timestamp"]),
                    point=WorldPoint(x=float(row["world_x"]), y=float(row["world_y"])),
                    source_pixel=PixelPoint(x=float(row["pixel_x"]), y=float(row["pixel_y"])),
                    projection_error_estimate_m=float(row["projection_error_estimate_m"]),
                    contact_quality=ContactPointQuality(row["contact_quality"]),

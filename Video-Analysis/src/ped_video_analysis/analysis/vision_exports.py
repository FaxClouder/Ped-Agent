...
from ped_video_analysis.vision.contracts import FrozenModel, ProcessedWorldTrackSet
...
def export_analysis_bundle(
    *,
    ...
    tables = {
        ...
        "density.csv": [item.model_dump(mode="json") for item in bundle.spatial.classic_density],
        "voronoi-density.csv": [
        ...
        "vector-field.csv": [item.model_dump(mode="json") for item in bundle.spatial.vector_field],
        "speed-profile.csv": [
            item.model_dump(mode="json") for item in bundle.spatial.speed_profile
        ],
        "fundamental-diagram.csv": [
            item.model_dump(mode="json") for item in bundle.spatial.fundamental_diagram
        ],
        "od.csv": [item.model_dump(mode="json") for item in bundle.od],
        "interactions.csv": [item.model_dump(mode="json") for item in bundle.interactions],
    }

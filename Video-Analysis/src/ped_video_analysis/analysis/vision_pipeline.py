...
from ped_video_analysis.analysis.pedpy_adapter import try_compute_pedpy_spatial
...
def _quality(source: ProcessedWorldTrackSet, profile: AnalysisProfile) -> QualitySummary:
    ...
    short_tracks = sum(
        1 for track in source.tracks if _duration(track) < profile.minimum_track_duration_seconds
    )
    expected_points = sum(
    ...
    return QualitySummary(
        ...
        degraded_point_count=sum(
            item.contact_quality in {ContactPointQuality.ESTIMATED, ContactPointQuality.FALLBACK}
...
def _velocity_at_frame(track: WorldTrack, frame: int) -> np.ndarray:
    ...
    if dt <= 0:
        return np.zeros(2)
    return np.array([(second.point.x - first.point.x) / dt, (second.point.y - first.point.y) / dt])

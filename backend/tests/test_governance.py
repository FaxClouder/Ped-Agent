...
def test_library_cli_exposes_manifest_quality_validation() -> None:
...
def test_cli_validates_regulation_manifest_with_regulation_quotas(tmp_path: Path) -> None:
    payloads: list[dict[str, object]] = []
    for index, record in enumerate(pilot_regulations(), start=1):
        source = tmp_path / f"regulation-{index}.pdf"
        source.write_bytes(f"%PDF-regulation-{index}".encode())
        payload = record.model_dump(mode="json")
        payload["source_path"] = str(source)
        payload["sha256"] = hashlib.sha256(source.read_bytes()).hexdigest()
        payloads.append(payload)
    manifest = tmp_path / "pilot-regulations.jsonl"
    manifest.write_text(
    ...
    result = CliRunner().invoke(
        app,
        ["library", "validate-manifest", str(manifest), "--phase", "pilot"],
    )

    assert result.exit_code == 0
    assert '"resource_count": 8' in result.stdout
